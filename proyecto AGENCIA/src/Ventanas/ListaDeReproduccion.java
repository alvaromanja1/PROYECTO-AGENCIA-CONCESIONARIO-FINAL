...
public class ListaDeReproduccion implements ListModel<String> {
	...
	int ficheroEnCurso = -1; // Fichero seleccionado (-1 si no hay ninguno
								// seleccionado)

	// Logger de la clase
	private static Logger logger = Logger.getLogger(ListaDeReproduccion.class
			.getName());
	 ...
	 * 
	 * @param posi
	 *            Posición del fichero en la lista (de 0 a size()-1)
	 ...
	 * carpeta indicada, que cumplan el filtro indicado. Si hay cualquier error,
	 * la lista de reproducción queda solo con los ficheros que hayan podido ser
	 * cargados de forma correcta.
	 * 
	 * @param carpetaFicheros
	 *            Path de la carpeta donde buscar los ficheros
	 * @param filtroFicheros
	 *            Filtro del formato que tienen que tener los nombres de los
	 *            ficheros para ser cargados. String con cualquier letra o
	 *            dígito. Si tiene un asterisco hace referencia a cualquier
	 *            conjunto de letras o dígitos. Por ejemplo p*.* hace referencia
	 *            a cualquier fichero de nombre que empiece por p y tenga
	 *            cualquier extensión.
	 * @return Número de ficheros que han sido añadidos a la lista
	 */
	public int add(String carpetaFicheros, String filtroFicheros) {
		// TODO: Codificar este método de acuerdo a la práctica (pasos 3 y
		// sucesivos)
		filtroFicheros = filtroFicheros.replaceAll("\\.", "\\\\."); // Pone el
																	// símbolo
																	// de la
																	// expresión
																	// regular
	 ...
	 * 
	 * @return true si la selección es correcta, false si hay error y no se
	 *         puede seleccionar
	 */
	 ...
	 * 
	 * @return Posición del fichero seleccionado en la lista de reproducción (0
	 *         a n-1), -1 si no lo hay
	 */
	...
	private void avisarAnyadido(int posi) {
		for (ListDataListener ldl : misEscuchadores) {
			ldl.intervalAdded(new ListDataEvent(this,
					ListDataEvent.INTERVAL_ADDED, posi, posi));
		}
	}
}